...
void MessageHelper::printMessage(const std::string &topic,
...
bool MessageHelper::parseFromZMQMsg(const zmq::message_t &msg,
...
bool MessageHelper::protobufToZMQMsg(zmq::message_t &msg,
                                     const google::protobuf::Message &message) {
  std::string payload;
...
Client::Client(const std::string &serverIP, const std::string &port,
...
sim::RPCServiceStatus Client::getMessageByTopic(const std::string &topic,
                                                std::string &payload) {
  // clear firstly
  payload.clear();
...
sim::RPCServiceStatus Client::setMessageByTopic(const std::string &topic,
                                                const std::string &payload) {
  sim::RPCServiceStatus serviceStatus;
  if (topic.empty()) {
    serviceStatus.set_state(1);
  ...
  zmq::message_t callInfoMsg(callInfoPayload.size() + 1),
      pairMsg(payloadPair.size() + 1);
  MessageHelper::protobufToZMQMsg(callInfoMsg, callInfo);
...
sim::RPCServiceStatus ServiceImpBase::dispatch(const sim::RPCCallInfo &callInfo,
  ...
  } else if (callInfo.funcname() ==
             std::string("getMessageByTopic_string_string")) {
...
sim::RPCServiceStatus ServiceImp_A::getMessageByTopic(const std::string &topic,
                                                      std::string &payload) {
  sim::RPCServiceStatus serviceStatus;
...
sim::RPCServiceStatus
ServiceImp_A::setMessageByTopic(const std::string &topic,
                                const std::string &payload) {
  sim::RPCServiceStatus serviceStatus;
