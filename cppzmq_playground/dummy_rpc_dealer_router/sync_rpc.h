...
class MessageHelper {
public:
  static void printMessage(const std::string &topic,
  ...
  static bool parseFromZMQMsg(const zmq::message_t &msg,
                              google::protobuf::Message &message);
  static bool protobufToZMQMsg(zmq::message_t &msg,
                               const google::protobuf::Message &message);
};

/**
 * @brief sync_rpc client,
 * send data format: id, RPCCallInfo, request_payload(protobuf or string,
 * service should know how to deal with it) reply data format: id, RPCCallInfo,
 * RPCServiceStatus, reply_payload(protobuf or string, client should know how to
 * deal with it)
 */
class Client {
public:
  Client(const std::string &serverIP, const std::string &port,
...
public:
  ...
  sim::RPCServiceStatus getMessageByTopic(const std::string &topic,
                                          std::string &payload);
  sim::RPCServiceStatus setMessageByTopic(const std::string &topic,
                                          const std::string &payload);
...
private:
  ...
  InternalState internalState_; // state of client, 0 means ok, 1 means warning,
                                // 2 means fatal
...
public:
  ...
  virtual sim::RPCServiceStatus getMessageByTopic(const std::string &topic,
                                                  std::string &payload) = 0;
  virtual sim::RPCServiceStatus
  setMessageByTopic(const std::string &topic, const std::string &payload) = 0;

public:
  sim::RPCServiceStatus dispatch(const sim::RPCCallInfo &callInfo,
                                 const zmq::message_t &msgReq,
...
public:
  ...
  virtual sim::RPCServiceStatus
  getMessageByTopic(const std::string &topic, std::string &payload) override;
  virtual sim::RPCServiceStatus
  setMessageByTopic(const std::string &topic,
